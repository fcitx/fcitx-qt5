...
void ListEditor::addFileTriggered() {
    ...
    std::FILE *file = FcitxXDGGetFileUserWithPrefix(
        QUICK_PHRASE_CONFIG_DIR, filename.append(".mb").toLocal8Bit(), "w",
        NULL);

    if (file) {
        fclose(file);
    } else {
        QMessageBox::warning(this, _("File Operation Failed"),
...
void ListEditor::removeFileTriggered() {
    ...
    char *fullname = NULL;
    FcitxXDGGetFileUserWithPrefix(NULL, filename.toLocal8Bit(), NULL,
                                  &fullname);
    QFile f(fullname);
    free(fullname);
    if (!f.exists()) {
        int ret = QMessageBox::question(
            ...
            _("%1 is a system file, do you want to delete all phrases instead?")
                .arg(curName),
    ...
    int ret =
        QMessageBox::question(this, _("Confirm deletion"),
